...
import { FormSelectPaymentMethod } from "@/components/forms/select-payment-method"
...
export default function TransactionEditForm({
  ...
  const transactionExtra = (transaction.extra ?? {}) as Record<string, unknown>
  ...
  const [formData, setFormData] = useState({
    ...
    gstAmount: transaction.gstAmount ? transaction.gstAmount / 100 : "",
    ...
    ...extraFields.reduce(
      (acc, field) => {
        acc[field.code] = toInputString(transactionExtra[field.code])
        return acc
      },
    ...
    <form action={saveAction} className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <FormSelectPaymentMethod
          title={fieldMap.paymentMethod?.name || "Payment Method"}
          name="paymentMethod"
          defaultValue={formData.paymentMethod}
          isRequired={fieldMap.paymentMethod?.isRequired || false}
        />

        <FormInput
          title={fieldMap.gstPercentage?.name || "GST %"}
          type="number"
          min="0"
          max="100"
          name="gstPercentage"
          defaultValue={String(formData.gstPercentage)}
          isRequired={fieldMap.gstPercentage?.isRequired || false}
        />

        <FormInput
          title={fieldMap.gstAmount?.name || "GST Amount"}
          type="number"
          step="0.01"
          name="gstAmount"
          defaultValue={String(formData.gstAmount)}
          isRequired={fieldMap.gstAmount?.isRequired || false}
        />
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        {extraFields.map((field) => (
          <FormInput
            ...
            title={field.name}
            name={field.code}
            defaultValue={toInputString(formData[field.code as keyof typeof formData])}
