...
export const createTransaction = async (userId: string, data: TransactionData): Promise<Transaction> => {
  const enrichedData = await enrichTransactionData(userId, data)
  const { standard, extra } = await splitTransactionDataExtraFields(enrichedData, userId)

  const newTransaction = await prisma.transaction.create({
    data: {
      ...standard,
      extra: extra,
      items: enrichedData.items as Prisma.InputJsonValue,
      userId,
    },
  })
...
export const updateTransaction = async (id: string, userId: string, data: TransactionData): Promise<Transaction> => {
  ...
  return await prisma.transaction.update({
    ...
    data: {
      ...standard,
      extra: extra,
      items: enrichedData.items ? (enrichedData.items as Prisma.InputJsonValue) : [],
    },
  })
}
...
const splitTransactionDataExtraFields = async (
  ...
  Object.entries(data).forEach(([key, value]) => {
    ...
    } else if (BUILTIN_TRANSACTION_FIELDS.has(key)) {
      standard[key] = value
    }
  })
...
const enrichTransactionData = async (userId: string, data: TransactionData): Promise<TransactionData> => {
  const detectionText = buildDetectionText(data)
  const enriched: TransactionData = { ...data }

  if (!enriched.currencyCode) {
    enriched.currencyCode = "INR"
  }

  if (!enriched.categoryCode && detectionText) {
    const detectedCategory = detectCategoryCode(detectionText)
    ...
    if (categoryExists?.code) {
      enriched.categoryCode = categoryExists.code
    } else {
      ...
      enriched.categoryCode = fallbackCategory?.code || null
    }
  }

  if (!enriched.paymentMethod && detectionText) {
    enriched.paymentMethod = detectPaymentMethod(detectionText)
  }

  if ((enriched.gstPercentage === null || enriched.gstPercentage === undefined) && detectionText) {
    enriched.gstPercentage = detectGstPercentage(detectionText)
  }

  if ((enriched.gstAmount === null || enriched.gstAmount === undefined) && detectionText) {
    enriched.gstAmount = detectGstAmount(detectionText)
  }
